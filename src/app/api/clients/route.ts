...
export async function POST(req: NextRequest) {
  try {
    ...
    db.prepare(`INSERT INTO clients (id, name, email, company_name, gstin, pan, financial_year, phone, address, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    ).run(clientId, name.trim(), email?.trim() || null, companyName.trim(), gstin?.trim() || null, pan?.trim() || null, financialYear, phone?.trim() || null, address?.trim() || null, now, now);
    try {
